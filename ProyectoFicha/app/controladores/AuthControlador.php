...
class AuthControlador {
        ...
        try {
            ...
            if ($usuario_datos) {
                if (password_verify($password, $usuario_datos['password'])) {
                    ...
                    $_SESSION['user_username'] = $usuario_datos['usuario'];
    ...
    public function logout() {
        try {
            // === INTEGRACIÓN PBX: Desactivar extensión telefónica ===
            if (isset($_SESSION['user_rol_id']) && (int)$_SESSION['user_rol_id'] === 2 && isset($_SESSION['user_username'])) {
                try {
                    require_once 'app/Servicios/PbxApiService.php';
                    $pbxService = new \App\Servicios\PbxApiService();
                    $pbxService->notificarLogout($_SESSION['user_username']);
                } catch (\Throwable $e) {
                    error_log("[PBX] Excepción en logout: " . $e->getMessage());
                }
            }
    ...
    /**
     * Proxy de heartbeat para recibir la llamada ajax del operador y mantener la sesión PBX.
     */
    public function heartbeatPbx() {
        header('Content-Type: application/json');
        
        // Verificar sesión activa de operador
        if (!isset($_SESSION['user_id']) || !isset($_SESSION['user_username']) || (int)$_SESSION['user_rol_id'] !== 2) {
            echo json_encode(['ok' => false, 'message' => 'Sesión no válida o no corresponde a operador.']);
            return;
        }
        ...
        try {
            require_once 'app/Servicios/PbxApiService.php';
            $pbxService = new \App\Servicios\PbxApiService();
            $result = $pbxService->enviarHeartbeat($_SESSION['user_username']);
            
            // Si el PBX reporta que no hay sesión activa (zombie/expirado en PBX por caída o corte), se auto-recupera
            if (isset($result['success']) && !$result['success'] && isset($result['http_code']) && $result['http_code'] === 404) {
                error_log("[PBX] Sesión no encontrada en heartbeat para {$_SESSION['user_username']}. Auto-recuperando sesión telefónica...");
                $loginRes = $pbxService->notificarLogin($_SESSION['user_username'], $_SESSION['user_name']);
                echo json_encode([
                    'ok' => $loginRes['success'],
                    'reauthenticated' => true,
                    'message' => $loginRes['success'] ? 'Sesión PBX restablecida.' : 'Fallo al restablecer sesión PBX.'
                ]);
                return;
            }
            ...
            echo json_encode([
                'ok' => $result['success'],
                'message' => $result['success'] ? 'Heartbeat recibido.' : ($result['error'] ?? 'Error desconocido')
            ]);
        } catch (\Throwable $e) {
            error_log("[PBX] Excepción en heartbeat proxy: " . $e->getMessage());
