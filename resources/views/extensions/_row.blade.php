<tr class="hover:bg-slate-50 transition group">
    ...
    <td class="px-6 py-4 text-right">
        <div class="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition">
            ...
            {{-- Eliminar/Deshabilitar (Modal Unificado) --}}
            <button type="button" @click="openDisable({{ $ext->id }}, '{{ $ext->numero }}', {{ $ext->is_active ? 'true' : 'false' }})" title="{{ $ext->is_active ? 'Deshabilitar' : 'Habilitar' }}" class="p-2 rounded-lg transition {{ $ext->is_active ? 'hover:bg-amber-50 text-amber-500 hover:text-amber-700' : 'hover:bg-emerald-50 text-emerald-500 hover:text-emerald-700' }}">
                @if($ext->is_active)
